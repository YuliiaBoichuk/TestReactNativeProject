...
class CreateTimeLinePost extends Component {
  ...
  createPost(){
    if(!this.state.form.Text){
      //Todo: ask Victor do we need handle only text empty or text|| (video&&image) and do we need set disabled button
      return;
    }
    ...
    post('/person/timeline/post', objToSend)
...
export default connect(mapStateToProps, bindActions)(CreateTimeLinePost);
